...
% preprocessFNIRS06_CV_GLM_ssBeta_PostVsAnt_MultiOnly

function grpPlotPerf_PostVsAnt_Fig6(goodSbj,numClasses,saveOp,itrOp)
...
%     sbjListPerfSTDHbT = zeros(numClassifiers,length(sbjList),length(timePt));
    ...
    for i = 1:length(sbjList)
        ...
        if numClasses == 2
            ...
            savePerfFN_PostVsAnt = 'performance_GLM_CV_SSBeta_LR_PostVsAnt_RejTr';
            ...
            savePerfFN_PostVsAnt = 'performance_GLM_CV_SSBeta_RejTr_PostVsAnt_';
        end
...
%             'performanceLDALedoitHbRFold','performanceLDALedoitHbTFold');
        ...
        load([processedDataDir filesep savePerfFN_PostVsAnt],...
            'performanceLDALedoitHbT_PFEF_1','performanceLDALedoitHbT_AFEF_1',...
...
%             [size(performance_RIPS_HbTFold,1), size(performance_RIPS_HbTFold,2)*size(performance_RIPS_HbTFold,3)]);
    ...
    grpPerfHbT = sum(sbjListPerfHbT,2) ./ sum(sbjListPerfHbT~=0,2);
    
    grpPerfHbTSE = squeeze(std(sbjListPerfHbT,[],2))./sqrt(sum(sbjListPerfHbT~=0,2));
    
    conf = 0.95;

    nSz = size(grpPerfHbTSE,2);
    grpPerfCIHbT = zeros(2,length(timeLen));
    
    for idxTime = 1:length(timeLen)

        grpPerfCIHbT(:,idxTime) = calcCITDist(nSz,grpPerfHbTSE(idxTime),conf);

    end

    cmap = loadDefaultColors(4);
    cmapROI = [cmap(1,:); cmap(2,:); cmap(2,:); cmap(3,:); cmap(3,:)];
    lineSty = {'-','-','--',':',':',':',':'};
    mrk = {'s','s','s','o','+','o','+'};
    %yLimAxis = [0 1];
...
%     errorbar(timePt./fs-2,performanceBoostedHbOMean,performanceBoostedHbOCI(:,2),'Color',cmap(6,:));
    ...
    for i = 1:5
        errorbar(timeLen./fs,grpPerfHbT(i,:),abs(grpPerfCIHbT(1,:)),abs(grpPerfCIHbT(2,:)),...
            'LineStyle',lineSty{i},'Marker',mrk{i},'Color',cmapROI(i,:));hold on;
    end
    ...
    %legend({'All','LFEF','RFEF','Post FEF 1','Ant FEF 1','Post FEF 2','Ant FEF 2'},'Location','Southeast');
    legend({'All','LFEF','RFEF','Posterior FEF','Anterior FEF'},'Location','Southeast');
...
%     end
    ...
    fn = sprintf('Fig6b_%sClass',num2str(numClasses));
