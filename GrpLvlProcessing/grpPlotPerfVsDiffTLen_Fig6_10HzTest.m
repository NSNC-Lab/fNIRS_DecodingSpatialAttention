...
function grpPlotPerfVsDiffTLen_Fig6_10HzTest(goodSbj,numClasses,saveOp,itrOp)
    ...
    sbjListPerfHbT = zeros(numSubsets,5*10*length(sbjList),length(timeLen));
    sbjListPerfHbT_10Hz = zeros(numSubsets,5*10*length(sbjList),length(timeLen));
...
%     sbjListPerfSTDHbT = zeros(numClassifiers,length(sbjList),length(timePt));
    ...
    for i = 1:length(sbjList)
        ...
        if numClasses == 2
            ...
            savePerfFN = 'performance_GLM_CV_SSBeta_LR_DiffTLen_RejTr';
            savePerfFN_10Hz = 'performance_GLM_CV_SSBeta_LR_DiffTLen_RejTr_10Hz';
...
%             [size(performance_RIPS_HbTFold,1), size(performance_RIPS_HbTFold,2)*size(performance_RIPS_HbTFold,3)]);
    ...
    %     test3 = mean(temp3(:));
        ...
        sbjListPerfHbT_10Hz(1,startIdx:endIdx,:) = performanceLDALedoitHbT_All;
        
        %sbjListPerfHbO(2,i,:) = mean(performanceLDALedoitHbO_LFEF,1);
        %sbjListPerfHbR(2,i,:) = mean(performanceLDALedoitHbR_LFEF,1);
        sbjListPerfHbT_10Hz(2,startIdx:endIdx,:) = performanceLDALedoitHbT_LFEF;
        
        %sbjListPerfHbO(3,i,:) = mean(performanceLDALedoitHbO_RFEF,1);
        %sbjListPerfHbR(3,i,:) = mean(performanceLDALedoitHbR_RFEF,1);
        sbjListPerfHbT_10Hz(3,startIdx:endIdx,:) = performanceLDALedoitHbT_RFEF;
        
        %sbjListPerfHbO(4,i,:) = mean(performanceLDALedoitHbO_LSTG,1);
        %sbjListPerfHbR(4,i,:) = mean(performanceLDALedoitHbR_LSTG,1);
        sbjListPerfHbT_10Hz(4,startIdx:endIdx,:) = performanceLDALedoitHbT_LSTG;
        
        %sbjListPerfHbO(5,i,:) = mean(performanceLDALedoitHbO_RSTG,1);
        %sbjListPerfHbR(5,i,:) = mean(performanceLDALedoitHbR_RSTG,1);
        sbjListPerfHbT_10Hz(5,startIdx:endIdx,:) = performanceLDALedoitHbT_RSTG;
        
        %sbjListPerfHbO(6,i,:) = mean(performanceLDALedoitHbO_LIPS,1);
        %sbjListPerfHbR(6,i,:) = mean(performanceLDALedoitHbR_LIPS,1);
        sbjListPerfHbT_10Hz(6,startIdx:endIdx,:) = performanceLDALedoitHbT_LIPS;
        
        %sbjListPerfHbO(7,i,:) = mean(performanceLDALedoitHbO_RIPS,1);
        %sbjListPerfHbR(7,i,:) = mean(performanceLDALedoitHbR_RIPS,1);
        sbjListPerfHbT_10Hz(7,startIdx:endIdx,:) = performanceLDALedoitHbT_RIPS;
    end
    ...
    grpPerfHbT = squeeze(sum(sbjListPerfHbT,2) ./ sum(sbjListPerfHbT~=0,2));
    grpPerfHbT_10Hz = squeeze(sum(sbjListPerfHbT_10Hz,2) ./ sum(sbjListPerfHbT_10Hz~=0,2));
    
    grpPerfHbTSE = squeeze(std(sbjListPerfHbT,[],2))./squeeze(sqrt(sum(sbjListPerfHbT~=0,2)));
    grpPerfHbTSE_10Hz = squeeze(std(sbjListPerfHbT_10Hz,[],2))./squeeze(sqrt(sum(sbjListPerfHbT_10Hz~=0,2)));
    
    conf = 0.95;

    nSz = size(grpPerfHbTSE,2);
    grpPerfCIHbT = zeros(numSubsets,2,length(timeLen));
    grpPerfCIHbT_10Hz = zeros(numSubsets,2,length(timeLen));
    
    for iSb = 1:numSubsets
        for idxTime = 1:length(timeLen)

            grpPerfCIHbT(iSb,:,idxTime) = calcCITDist(nSz,grpPerfHbTSE(iSb,idxTime),conf);
            grpPerfCIHbT_10Hz(iSb,:,idxTime) = calcCITDist(nSz,grpPerfHbTSE_10Hz(iSb,idxTime),conf);
    ...
    figure();hold on;
    errorbar(timeLen./fs,grpPerfHbT(1,:),squeeze(abs(grpPerfCIHbT(1,1,:))),squeeze(abs(grpPerfCIHbT(1,2,:))),...
        'Color',cmapROI(1,:),'LineStyle',lineStyleROI{1});
    errorbar(timeLen./fs,grpPerfHbT_10Hz(1,:),squeeze(abs(grpPerfCIHbT_10Hz(1,1,:))),squeeze(abs(grpPerfCIHbT_10Hz(1,2,:))),...
        'Color',cmapROI(1,:),'LineStyle',lineStyleROI{3});
    legend({'Original','10 Hz'});
    title('All Channel');
    ylim([0.3 1]);
    ylabel('Accuracy');
    ...
    figure();hold on;
    errorbar(timeLen./fs,grpPerfHbT(2,:),squeeze(abs(grpPerfCIHbT(2,1,:))),squeeze(abs(grpPerfCIHbT(2,2,:))),...
        'Color',cmapROI(2,:),'LineStyle',lineStyleROI{1});
    errorbar(timeLen./fs,grpPerfHbT_10Hz(2,:),squeeze(abs(grpPerfCIHbT_10Hz(2,1,:))),squeeze(abs(grpPerfCIHbT_10Hz(2,2,:))),...
        'Color',cmapROI(2,:),'LineStyle',lineStyleROI{3});
    legend({'Original','10 Hz'});
    title('Left FEF');
    ylim([0.3 1]);
    ylabel('Accuracy');
    ...
    figure();hold on;
    errorbar(timeLen./fs,grpPerfHbT(3,:),squeeze(abs(grpPerfCIHbT(3,1,:))),squeeze(abs(grpPerfCIHbT(3,2,:))),...
        'Color',cmapROI(3,:),'LineStyle',lineStyleROI{1});
    errorbar(timeLen./fs,grpPerfHbT_10Hz(3,:),squeeze(abs(grpPerfCIHbT_10Hz(3,1,:))),squeeze(abs(grpPerfCIHbT_10Hz(3,2,:))),...
        'Color',cmapROI(3,:),'LineStyle',lineStyleROI{3});
    legend({'Original','10 Hz'});
    title('Right FEF');
    ylim([0.3 1]);
    ylabel('Accuracy');
...
%     errorbar(timePt./fs-2,performanceBoostedHbOMean,performanceBoostedHbOCI(:,2),'Color',cmap(6,:));
    ...
    for i = 1:numSubsets-2
        errorbar(timeLen./fs,grpPerfHbT(i,:),abs(grpPerfCIHbT(1,:)),abs(grpPerfCIHbT(2,:)),...
            'Color',cmapROI(i,:),'LineStyle',lineStyleROI{i});
    end
