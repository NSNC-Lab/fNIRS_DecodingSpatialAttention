...
% 
% SYNTAX:
% preprocessFNIRS06_LOFO_MultiOnly(s,numClasses,rejTrOp,rejChnOp,...
%   fefOnlyOp,rerunOp,plotOp)
% 
% DESCRIPTION:
% Performs leave-one-feature-out classification on entire dataset (using
%   Markov Blanket).
%   Otherwise identical. Repeated description here:
...
%       1 - remove channels
% fefOnlyOp - int: test only frontal eye field (FEF) subset.
%       0 - test entire probe.
%       1 - test only FEF subset.
...
%   decision window lengths.

function preprocessFNIRS06_LOFO(s,numClasses,rejTrOp,rejChnOp,fefOnlyOp,rerunOp,plotOp)
...
%         0:1/50:(size(snirf1.data.dataTimeSeries,1)-1)/50,snirf1.data.measurementList);
    ...
    performanceLDALedoitHbT_All = zeros(nrept*kFold,1);
    performanceLDALedoitHbT_LOFO = zeros(nrept*kFold,size(idxChns,2));
...
%                 indexMoviesTrainMultiple,indexMoviesTestMultiple,numClasses,mlList_FEF,timeLen);
            ...
            performanceLDALedoitHbT_All(foldIdx) = train_RLDA_Ledoit_TrTst(cumsumTr,cumsumTst,mlList,numChn,numClasses,indexMoviesTrainMultiple,indexMoviesTestMultiple);

            % Left FEF
            performanceLDALedoitHbT_LOFO(foldIdx,:) = ...
                calcCumSum_DiffStartT_AllChn_CompClassifiers_NoSave_LOFO(sbjNum,trials_HbTM_Tr,trials_HbTM_Tst,...
                indexMoviesTrainMultiple,indexMoviesTestMultiple,numClasses,idxChns,mlList);
    ...
    if numClasses == 2
        if fefOnlyOp
            fileName = [processedDataDir filesep 'performance_LOFO_FEF_LR_10Hz.mat'];
        else
            fileName = [processedDataDir filesep 'performance_LOFO_LR_10Hz.mat'];
        end
    else
        if fefOnlyOp
            fileName = [processedDataDir filesep 'performance_LOFO_FEF.mat'];
        else
            fileName = [processedDataDir filesep 'performance_LOFO.mat'];
        end
    end

    save(fileName,'performanceLDALedoitHbT_All','performanceLDALedoitHbT_LOFO');
    
else
    if numClasses == 2
        if fefOnlyOp
            fileName = [processedDataDir filesep 'performance_LOFO_FEF_LR.mat'];
        else
    ...
    load(fileName,'performanceLDALedoitHbT_LOFO','performanceLDALedoitHbT_LOFO');
