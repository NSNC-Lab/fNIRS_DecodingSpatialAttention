...
if strcmp(sbjNum,'15')
    ...
    idx = idx(2:end);
    rejTr = find(~idx);
    cueOnsetIndex = cueOnsetIndex(2:end);
else
    rejTr = find(~idx);
end

for i = 1:length(rejTr)
    dod.dataTimeSeries((allS(cueOnsetIndex(rejTr(i)),1)+trialTime(1))*fs:(allS(cueOnsetIndex(rejTr(i)),1)+trialTime(2))*fs,:) = 0;
end
...
if numClasses==2
    ...
    indexMoviesTestCond = indexMoviesTest(indexMoviesTest(:,5)==1,:);
    numTr = sum(logical(idx));
    %indexMoviesTestCond(:,6)=1:size(indexMoviesTestCond,1);
end
...
%     testStim = allS(cv.test(i));
    ...
    snirfTemp.stim(1,1) = leftOnsetM;
    snirfTemp.stim(1,2) = rightOnsetM;
    snirfTemp.stim(1,3) = centerOnsetM;
...
%     end
    ...
    %for i2 = 1:1
        ...
        if indexMoviesTest(i2,5) == indexMoviesTest(i1,5)
            ...
            dcTrAvgReshape = reshape(dcTrAvg.dataTimeSeries,size(dcTrAvg.dataTimeSeries,1),3,origNumChn,3);
            ...
            dcTrial = DataClass(dc.dataTimeSeries((allSTemp(cueOnsetIndexTemp(i2),1)+trialTime(1))*fs:(allSTemp(cueOnsetIndexTemp(i2),1)+trialTime(2)+1)*fs,:),...
                dc.time((allSTemp(cueOnsetIndexTemp(i2),1)+trialTime(1))*fs:(allSTemp(cueOnsetIndexTemp(i2),1)+trialTime(2)+1)*fs,1),...
                dc.measurementList);
            ...
            tIncAutoTrial{1} = ones(size(dcTrial.time,1),1);
...
if numClasses==2
    ...
    fileName = [processedDataDir filesep 'GLM_CV_Performances_3Classes.mat'];
end
