...
function grpPlotHRF_Version02(sbjList)
...
%                     plot(t,indHb(:,chnIdx,k),'Color',colorListLight{colorIdx(i2)});hold on;
%                 end
%             end
        ...
        for i2 = 1:3
            % dcAvg.dataTimeSeries is time x channels
%             if opPlotInd
%                 for j = 1:length(sbjList)
%                     chnIdx = sizeCond*(condSubPlotIdx(i2)-1) + sizeChn*(srcIdxGrp{i1}(j)-1) + 2;
%                     plot(t,indHb(:,chnIdx,j),'Color',colorListLight{colorIdx(i2)});hold on;
%                 end
%             end
