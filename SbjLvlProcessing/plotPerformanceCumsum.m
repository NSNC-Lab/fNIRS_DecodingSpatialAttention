...
function plotPerformanceCumsum(sbjNum,numClasses,behScore)
...
if numClasses == 2
    load([saveDir filesep 'perfLinearDiscriminantUpdated_LR_Basis1.mat'],'performanceArrMultiHbO','performanceArrSingleHbO',...
    ...
    load([saveDir filesep 'perfLinearDiscriminantUpdated_Basis1.mat'],'performanceArrMultiHbO','performanceArrSingleHbO',...
        'performanceArrMultiHbR','performanceArrSingleHbR',...
        'performanceArrMultiHbT','performanceArrSingleHbT',...
        'predRespSHbO','predRespMHbO','singleTrialHRFHbOS','singleTrialHRFHbOM',...
        'singleTrialHRFHbRS','singleTrialHRFHbRM',...
...
for j = 1:size(performanceArrSingleHbO,1)
    ...
    plot(timePt./fs-2,performanceArrSingleHbO(j,:),'Color',cmap(j,:));
end
ylim([0 1]);
...
for j = 1:size(performanceArrSingleHbR,1)
    %plot(timePt./fs-2,performanceArrSingle(sizeChn*(j-1) + 1,:));
    plot(timePt./fs-2,performanceArrSingleHbR(j,:),'Color',cmap(j,:));
end
ylim([0 1]);
title(sprintf('Sbj %s: Δ[HbR] Single Movie',num2str(sbjNum)));
%legend('S1D1','S1D2','S1D3','S1D4');
xlabel('Time [s]');ylabel('Accuracy');
hold off;
...
ylim([0 1]);
title(sprintf('Sbj %s: Δ[HbR] Multi Movie',num2str(sbjNum)));
%legend('S1D1','S1D2','S1D3','S1D4');
xlabel('Time [s]');ylabel('Accuracy');
hold off;
...
for j = 1:size(performanceArrSingleHbT,1)
    %plot(timePt./fs-2,performanceArrSingle(sizeChn*(j-1) + 1,:));
    plot(timePt./fs-2,performanceArrSingleHbT(j,:),'Color',cmap(j,:));
end
ylim([0 1]);
title(sprintf('Sbj %s: Δ[HbT] Single Movie',num2str(sbjNum)));
%legend('S1D1','S1D2','S1D3','S1D4');
xlabel('Time [s]');ylabel('Accuracy');
hold off;
...
ylim([0 1]);
title(sprintf('Sbj %s: Δ[HbT] Multi Movie',num2str(sbjNum)));
%legend('S1D1','S1D2','S1D3','S1D4');
xlabel('Time [s]');ylabel('Accuracy');
...
legend(LSChnList(1:30));
annotation('textbox', [0.8, 0.1, 0.1, 0.1], 'String', ['Score: ' num2str(behScore) '%']);
hold off;
