...
function [data_yavg, data_yavgstd, nTrials, data_ynew, data_yresid, data_ysum2, ...
    ...
    hmrR_GLM_ssBeta(data_y, stim, probe, mlActAuto, Aaux, tIncAuto, rcMap, trange, glmSolveMethod, idxBasis, paramsBasis, rhoSD_ssThresh, flagNuisanceRMethod, driftOrder, c_vector)
...
for iBlk=1:length(data_y)
    ...
    ssBeta = zeros(nB*nCond+driftOrder+1+nAux+nMC,nCh,2);
    for conc=1:2 %only HbO and HbR
        ...
        for iSS = 1:length(mlSSlst)
            ...
            if ~isempty(lstML)
                ...
                % ytmp is measured fNIRS signals.
                % At contains both HRFs and systemic physiology.
                % foo is beta coefficients for both HRFs and systemic
                % physiology.
                % dA contains only HRFs basis.
                % At contains HRFs basis + SS regressors + drifting poly +
                % tCCA regressors
                % so ynew contains just HRFs plus residual.
                % which is equal to trial minus SS regressors.
