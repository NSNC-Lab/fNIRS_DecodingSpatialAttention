...
function grp_Plot_Subset_CumsumNDelta(sbjGrp,numClasses,fefOnlyOp,saveOp)
...
performance_All = zeros(5*10*length(sbjList),length(timeLen));
performance_Subset  = zeros(5*10*length(sbjList),length(timeLen));
performance_Delta = zeros(5*10*length(sbjList),length(deltaLen));
performance_Delta1  = zeros(5*10*length(sbjList),length(deltaLen));
...
for i = 1:length(sbjList)
    ...
    load(fileName,'performanceLDALedoitHbT_All','performanceLDALedoitHbT_LOFO','performanceLDALedoitHbT_Final',...
    ...
    performance_All(startIdx:endIdx,:) = performanceLDALedoitHbT_All;
    performance_Subset(startIdx:endIdx,:) = performanceLDALedoitHbT_Final;
    performance_Delta(startIdx:endIdx,:) = performanceLDALedoitHbT_Delta;
    performance_Delta1(startIdx:endIdx,:) = performanceLDALedoitHbT_Delta1;
...
errorbar(timeLen./fs,performance_All_Mean,performanceLDALedoitHbT_All_CI(:,2));hold on;
errorbar(timeLen./fs,performance_Subset_Mean,performanceLDALedoitHbT_Subset_CI(:,2));hold on;
...
legend('All','Subset','Delta 0s','Delta 1s');

if saveOp == 1
    
    if fefOnlyOp
        fn = sprintf('Grp_Subset_Delta_HbT');
    else
        fn = sprintf('Grp_Subset_Delta_HbT');
    end
