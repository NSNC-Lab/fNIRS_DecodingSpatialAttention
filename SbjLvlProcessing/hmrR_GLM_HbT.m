...
%       betaList - list of beta index to model. For T-Test, for null hypothesis,
%           if Bi = 0, then exclude ith beta from betaList array. Example, 
%           for null hypothesis B4 = B5 = 0, then betaList = [1 2 3 6 7 8].
%           only work for idxBasis==1
%       condList - list of condition to model. Example, for null hypothesis
...
function [data_yavg, data_yavgstd, nTrials, data_ynew, data_yresid, data_ysum2, ...
    ...
    hmrR_GLM_HbT(data_y, stim, probe, mlActAuto, Aaux, tIncAuto, rcMap, trange, glmSolveMethod, idxBasis, paramsBasis, rhoSD_ssThresh, flagNuisanceRMethod, driftOrder, c_vector, betaF)
...
%     foo = nTrials{iBlk};
%     nTrials{iBlk} = zeros(1,nCond);
%     nTrials{iBlk}(lstCond) = foo;
%     
%     foo = yavg;
%     yavg = zeros(size(foo,1),size(foo,2),size(foo,3),nCond);
%     yavg(:,:,:,lstCond) = foo;
%     
%     foo = yavgstd;
%     yavgstd = zeros(size(foo,1),size(foo,2),size(foo,3),nCond);
%     yavgstd(:,:,:,lstCond) = foo;
%     
%     foo = ysum2;
%     ysum2 = zeros(size(foo,1),size(foo,2),size(foo,3),nCond);
%     ysum2(:,:,:,lstCond) = foo;
