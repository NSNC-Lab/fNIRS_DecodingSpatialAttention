% Matthew Ning's note: modified to accept any vector as basis, used for CV
% To use that option, set idxBasis = 5 and paramsBasis to the vector
% glmSolveMethod= 1 is the only option supported for the above parameters.
% flagNuisanceRMethod does not depend on above options
% 
% Make sure that the calcDWTMatrix.m or other function calculate and return
% all of the below outputs.

% SYNTAX:
% [At] = hmrR_GLM(data, stim, probe, mlActAuto, Aaux, tIncAuto, trange, rcMap, glmSolveMethod, idxBasis, paramsBasis, rhoSD_ssThresh, flagNuisanceRMethod, driftOrder, c_vector)
%
% UI NAME:
% GLM_HRF_Drift_SS
%
...
function [A, lstML, mlSSlst] = ...
    hmrR_GLM_BuildDesignMatrix_CVTest(data_y, stim, probe, mlActAuto, Aaux, tIncAuto, rcMap, trange, glmSolveMethod, idxBasis, paramsBasis, rhoSD_ssThresh, flagNuisanceRMethod, driftOrder, c_vector)
...
for iBlk=1:length(data_y)
    ...
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    if idxBasis ~= 5
        dA=zeros(nT,nB*nCond,2); % nT X # weights X chromophores
        for iConc = 1:2
            iC = 0;
            for iCond=1:nCond
                for b=1:nB
    ...
    elseif idxBasis == 5
        dA=zeros(nT,nB*nCond,2,size(paramsBasis,3)); % nT X # weights X chromophores X chns
        for iConc = 1:2
            for iCh = 1:size(paramsBasis,3)
                iC = 0;
                for iCond=1:nCond
                    for b=1:nB
                        ...
                        % MN: channel indexing not needed but verify first
                        beta_label{b + (iCond-1)*nB} = ['Cond' num2str(iCond)];
                    end
                end
            end
...
%                 end
                ...
                if glmSolveMethod==1 % ~flagUseTed
                    ...
                    elseif idxBasis == 5
                        ...
                        for i = 1:size(lstML,1)
                            ...
                            % ATA(ATA)^-1AT = AT
                            % AT = AT]
                            % This is beta weight, the OLS solution
                ...
                if idxBasis ~= 5
                    for iCond=1:nCond
                        % nB is num of basis for Gaussian basis
                ...
                elseif idxBasis == 5 % OLS only
                    ...
                    for i = 1:size(lstML,1)
                        ...
                        tval(:,lstML(i),conc) =  foo(:,lstML(i),conc)./sqrt(bvar(:,lstML(i),conc));
                        pval(:,lstML(i),conc) = 1-tcdf(abs(tval(:,lstML(i),conc)),(size(y,1)-1));
                        for iCond=1:nCond
                            %                yavgstd(:,lstML(iCh),conc,iCond) = diag(tbasis*diag(bvar([1:nB]+(iCond-1)*nB,lstML(iCh),conc))*tbasis').^0.5;
                            if size(tbasis,3)==1
                            ...
                            ysum2(:,lstML(i),conc,iCond) = yavgstd(:,lstML(i),conc,iCond).^2 + nTrials{iBlk}(iCond)*yavg(:,lstML(i),conc,iCond).^2;
                        end
                    end
                end
    ...
    % time X weight X channels X conds
    foo = yavg;
    yavg = zeros(size(foo,1),size(foo,2),size(foo,3),size(s,2));
    yavg(:,:,:,lstCond) = foo;
