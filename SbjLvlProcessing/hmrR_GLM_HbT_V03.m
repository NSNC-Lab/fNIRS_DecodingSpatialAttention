% 4/11/2022: replace nB/nCond loop with totNumCoeffs loop
%
% 4/4/2022: Updated to calculate GLM fitting to HbT data
% Never tested for glmSolveMethod == 2, but should work
% 
% Note: only work with idxBasis==1
%
% Both y_CI and beta_CI can be derived from yavgstd and beta, respectively.
%
% Add support for F-Test for joint significance test. You can also use
% t-test by using contrast vector. Both are valid but T-test allow you to
...
%           [0 1 -1 0]
%       OR
%   struct. Only for full model (so betaF is empty). For reduced model, leave it empty.
%           If use betaT, leave c_vector empty.
...
%       condList - list of conditions to model. Example, for null hypothesis
%           left = 0, then betaS.condList = [0 1 1] (left, right, center).
%           Can list multiple condition lists. [0 1 -1;1 0 -1;1 -1 0]
% betaF - struct. Only for reduced model. For full model, leave it empty.
%       betaList - list of beta index to model. For F-Test, for null hypothesis,
%           if Bi = 0, then include ith beta in betaList array. Example, 
%           for null hypothesis B4 = B5 = 0, then betaList = [4 5].
%           only work for idxBasis==1. If use other idxBasis, will ignore
%           this variable.
%       condList - list of condition to exclude. Example, for null hypothesis
%           left = 0, then betaS.condList = [0 1 1] (left, right, center).
%           If betaList is null, then all beta coefficients in that
%           condition will be exluded. Else, only beta index for exluded
...
%     (#Channels x HbX)
% bvar - beta coefficient variance term, the diagonal entries of covariance
%       matrix. May update to return full matrix in the future.
...
% SSE - sum of squared errors. For F-Test.
%
% USAGE OPTIONS:
...
function [data_yavg, data_yavgstd, nTrials, data_ynew, data_yresid, data_ysum2, ...
    ...
    hmrR_GLM_HbT_V02(data_y, stim, probe, mlActAuto, Aaux, tIncAuto, rcMap, trange, glmSolveMethod, idxBasis, paramsBasis, rhoSD_ssThresh, flagNuisanceRMethod, driftOrder, c_vector, betaF)
...
% cannot have both c_vector and betaF because will confuse very easily.
if exist('c_vector','var') && exist('betaF','var')
    return;
end
...
for iBlk=1:length(data_y)
    ...
    origLstCond = lstCond;
    if exist('betaF','var')
        if isempty(betaF.betaList)
            condList = betaF.condList;
            nCond = sum(condList);
            totNCond = size(condList,2);
            origLstCond(~condList) = [];
        else
            condList = betaF.condList;
        ...
        condList = ones(1,length(lstCond));
        nCond = length(lstCond); %size(s,2);
        totNCond = nCond;
    end
    onset = zeros(nT,totNCond);
    nTrials{iBlk} = zeros(totNCond,1);
    ...
    if idxBasis==1
        ...
        for b=1:nB
            thisB = betaList(b);
            tbasis(:,b) = exp(-(tHRF-(trange(1)+thisB*gms)).^2/(2*gstd.^2));
    ...
    elseif idxBasis==2
        % Modified Gamma
        if exist('betaF','var')
            warning('betaF is only valid for idxBasis == 1. betaF will be ignored.');
        end
        nConc = 2;
        nB = 1;
    ...
    % determine total number of beta coefficients for HRFs
    if ~exist('betaF','var')
        totNumCoeffs = nB*nCond;
    elseif isfield(betaF,'betaList')
        totNumCoeffs = nB*nCond;
    else
        totNumCoeffs = length(betaList)*sum(~betaF.condList)+nB*sum(betaF.condList);
    end
    
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    % Construct design matrix
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    dA=zeros(nT,totNumCoeffs,3);
    if ~exist('betaF','var') || (exist('betaF','var') && isfield(betaF,'betaList'))
        for iConc = 1:3
            iC = 0;
            ithCond = 0;
            for iCond=1:totNCond
        ...
        for iConc = 1:3
            ...
            for iCond=1:totNCond
                for b=1:nB
                    iC = iC + 1;
                    if ~(~betaF.condList(iCond) && ismember(b,betaF.betaList))
    ...
    A = zeros(nT,nB*nCond+driftOrder+nAux+nMC,3);
    ...
    ynew    = zeros(nT,3,nCh);
    yR      = zeros(nCh,3);
    foo     = zeros(nB*nCond+driftOrder+1+nAux+nMC,nCh,3); % 4 extra for 3rd order drift + nAux
    bvar    = zeros(nB*nCond+driftOrder+1+nAux+nMC,nCh,3);
    ssBeta  = zeros(nB*nCond+driftOrder+1+nAux+nMC,nCh,3);
    yvar    = zeros(1,nCh,3);
    sse     = zeros(1,nCh,3);
    yest    = zeros(nT,nCh,3);
    
    % should be the same regardless of channel or concentration
    dof = size(y,1)-(nB*nCond+1+driftOrder+nAux+nMC);
    contrastVal = [1 -1 0];
    % old code
    % dof = size(y,1) - 1.
    
    if isa(c_vector,'double')
        tval_contrast = zeros(size(c_vector,1),nCh,3);
        pval_contrast = zeros(size(c_vector,1),nCh,3);
    elseif isa(c_vector,'struct')
        tval_contrast = zeros(size(c_vector.condList,1),nCh,3);
        pval_contrast = zeros(size(c_vector.condList,1),nCh,3);
    end
    for conc=1:3
        ...
        for iSS = 1:length(mlSSlst)
            ...
            if ~isempty(lstML)
                ...
                if glmSolveMethod==1 % ~flagUseTed
                    ...
                    foo(:,lstML,conc)=pinvA*squeeze(ytmp); % beta coefficients
                elseif glmSolveMethod==2
                    ...
                    for chanIdx=1:length(lstML)
                        ...
                        [dmoco, beta_ar, tstat, pval, sigma, CovB, dfe, w, P, f] = ar_glm_final(squeeze(ytmp2),At(lstInc,:));
                        foo(:,lstML(chanIdx),conc)=beta_ar;
                ...
                if glmSolveMethod==1 %  OLS  ~flagUseTed
                    % we only care about variance and not covariance, hence
                    % diag term.
                    % (X'*X)^-1 = (X'^-1)*(X^-1)
                    %pAinvAinvD = diag(pinvA*pinvA');
                    pAinvAinvD = diag(pinvA*pinvA');
                    yest(:,lstML,conc) = At * foo(:,lstML,conc);
                    % This represents sample error variance term, as well as
                    % conditional variance of y given x, beta and error.
                    % don't confuse error with residual.
                    ...
                    for iCh = 1:length(lstML)
                        ...
                        %b_ci(:,lstLM(iCh),conc) = tinv(1-((1-0.95)/2),dof)*sqrt(bvar(:,lstML(iCh),conc));
                        ...
                        % equation 4 of Huppert 2016 Neurophotonics paper
                        if nCond > 1
                            ...
                            if isa(c_vector,'double')
                                ...
                                if (sum(abs(c_vector)) ~= 0) && (size(c_vector,2) == nCond)
                                    for irow = 1:size(c_vector,1)
                                        ...
                                        cv_dummy = [];
                                        for m = 1:nCond
                                            cv_dummy = [cv_dummy ones(1,nB)*c_row(m)];
                                        end
                            ...
                            elseif isa(c_vector,'struct')
                                for i_row = 1:size(c_vector.condList,1)
                                    temp_CondList = c_vector.condList(i_row,:);
                                    p = ismember(temp_CondList,contrastVal);
                                    maxB = max(c_vector.betaList);
                                    minB = min(c_vector.betaList);
                                    if isempty(temp_CondList(~p)) && maxB <= nB && minB > 0
                                        cv_extended = zeros(1,size(beta_label,2));
                                        for m = 1:nCond
                                            if c_vector.condList(i_row,m) ~= 0
                                                cv_extended(1,(m-1)*nB+abs(c_vector.condList(i_row,m))*c_vector.betaList) = c_vector.condList(i_row,m);
                                            end
                                        end
                                        ...
                                        pval_contrast(i_row,lstML(iCh),conc) = 1-tcdf(abs(tval_contrast(i_row,lstML(iCh),conc)),(size(y,1)-1));
                                    end
                                end
                            end
                        end
                        ...
                        % SE of the mean = sqrt(var)/sqrt(n) = std/sqrt(n)
                        for iCond=1:nCond
                            if size(tbasis,3)==1
                                % this is unconditional standard deviation
...
%     foo = tb;
%     beta = zeros(size(foo,1),size(foo,2),size(foo,3),nCond);
%     beta(:,:,:,lstCond) = foo;
    beta = tb;
