% Set SS Beta Prior = 1. No GLM here.
%
% For sbj 12 and after. For sbj 08 and 10, use
% preprocessFNIRS06_CV_ssBeta.m

function preprocessFNIRS06_CV_ssBeta_MultiOnly(sbjNum,rawDataFN,respData)
...
figSaveDir = ['C:\Users\mn0mn\Documents\ResearchProjects\spatailAttentionProject\ProcessedDatafNIRS\Experiment' num2str(sbjNum) '\Figures'];
...
AddStims(leftOnsetM, allS1(cueOnsetIndex(leftMultiIndex)));
AddStims(rightOnsetM, allS1(cueOnsetIndex(rightMultiIndex)));
AddStims(centerOnsetM, allS1(cueOnsetIndex(centerMultiIndex)));
...
% dc.dataTimeSeries is time x (chn x conc)
% idx 1 is HbO, idx 2 is HbR, idx 3 is HbT, idx 4 is HbO
dc = hmrR_OD2Conc(dod,probe,[1  1  1]);

% beta is 16 x 2 x 36 x 3
% ssBeta is 49 x 36 x 2
...
snirf1.Save([saveDir filesep rawDataFN{1} 'ssBetaPrior.snirf']);
...
save(fileName,'dc','dcNew','dod','stim','tRange',...
    'tIncAuto','mlActAuto','tIncAuto','snirf1','allS1');
