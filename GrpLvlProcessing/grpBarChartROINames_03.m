...
function grpBarChartROINames_03(sbjList,numRept,numClasses,rerunOp,saveOp)
...
grpMeanPerfHbT = mean(nonzeros(varPerfHbTGrp));

for i = 1:size(chnGrp,2)
    grpMeanPerfHbO(i) = mean(nonzeros(varPerfHbO(:,i)));
    grpMeanPerfHbR(i) = mean(nonzeros(varPerfHbR(:,i)));
    grpMeanPerfHbT(i) = mean(nonzeros(varPerfHbTGrp(:,i)));
end

%figure();
X = categorical(chnGrpName);
X = reordercats(X,chnGrpName);

save([figDir filesep 'grpBarChartGrpROINames.mat']);
...
for i1 = 1:size(chnGrp,2)
    tempY = varPerfHbTGrp(:,i1);
    tempX = i1*ones(1,length(sbjList));
    for i2 = 1:length(sbjList)
        plot(tempX(i2),tempY(i2),'o','MarkerFaceColor',colorCodes(i2,:),'MarkerEdgeColor',colorCodes(i2,:),'MarkerSize',5);
    end
...
ax = gca;
ax.XTick = 1:1:size(chnGrpName,2);
ax.XTickLabels = chnGrpName;
xtickangle(45);
...
if saveOp == 1
    ...
    fn = sprintf('Grp_CVPerfByGrpROINames_HbT');
