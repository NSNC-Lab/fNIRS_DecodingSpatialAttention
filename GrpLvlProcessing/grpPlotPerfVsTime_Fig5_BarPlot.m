...
function grpPlotPerfVsTime_Fig5_BarPlot(numClasses,saveOp,ciOp)
    ...
    %goodGrpPerfHbTAllSbj = zeros(length(timePt),length(goodSbjList));
    ...
    %badGrpPerfHbTAllSbj = zeros(length(timePt),length(badSbjList));
    ...
    %allGrpPerfHbTAllSbj = zeros(length(timePt),length(allSbjList));
    ...
    %cellGrpPerfHbTIndSbj = {goodGrpPerfHbTAllSbj,badGrpPerfHbTAllSbj,allGrpPerfHbTAllSbj};
    ...
    sbjCellPerfHbO = cell(1,3);
    sbjCellPerfHbR = cell(1,3);
    sbjCellPerfHbT = cell(1,3);
    
    sbjListGoodPerfHbO = zeros(length(goodSbjList),length(timePt));
    sbjListGoodPerfHbR = zeros(length(goodSbjList),length(timePt));
    sbjListGoodPerfHbT = zeros(length(goodSbjList),length(timePt));
    
    sbjListBadPerfHbO = zeros(length(badSbjList),length(timePt));
    sbjListBadPerfHbR = zeros(length(badSbjList),length(timePt));
    sbjListBadPerfHbT = zeros(length(badSbjList),length(timePt));
...
%                 cellSbjListPerfHbT{i1}(1,i2,:) = mean(performanceLDALedoitHbT,1);
                ...
                sbjCellPerfHbO{i1}(i2,:) = mean(performanceLDALedoitHbO,1);
                sbjCellPerfHbR{i1}(i2,:) = mean(performanceLDALedoitHbR,1);
                sbjCellPerfHbT{i1}(i2,:) = mean(performanceLDALedoitHbT,1);
                ...
                if i1 == 1
                    sbjListGoodPerfHbO(i2,:) = mean(performanceLDALedoitHbO,1);
                    sbjListGoodPerfHbR(i2,:) = mean(performanceLDALedoitHbR,1);
                    sbjListGoodPerfHbT(i2,:) = mean(performanceLDALedoitHbT,1);
                end
                
                if i1 == 2
                    sbjListBadPerfHbO(i2,:) = mean(performanceLDALedoitHbO,1);
                    sbjListBadPerfHbR(i2,:) = mean(performanceLDALedoitHbR,1);
                    sbjListBadPerfHbT(i2,:) = mean(performanceLDALedoitHbT,1);
                end
            ...
            elseif ciOp == 2
                grpPerfHbOAllFoldsSE{i1} = squeeze(std(sbjCellPerfHbO{i1},[],1))./sqrt(size(sbjCellPerfHbO{i1},1));
                grpPerfHbRAllFoldsSE{i1} = squeeze(std(sbjCellPerfHbR{i1},[],1))./sqrt(size(sbjCellPerfHbR{i1},1));
                grpPerfHbTAllFoldsSE{i1} = squeeze(std(sbjCellPerfHbT{i1},[],1))./sqrt(size(sbjCellPerfHbT{i1},1));
            end
    ...
    % for HbX one way ANOVA test, followed by post-hoc multiple comparisons
%     dHbX = [cellGrpPerfHbO{3}(tidx,:); cellGrpPerfHbR{3}(tidx,:); cellGrpPerfHbT{3}(tidx,:)];
%     [p,~,stats] = anova1(dHbX');
%     [results,means,~,gnames] = multcompare(stats,"CType","bonferroni");
    [p,~,stats] = anova1([sbjListPerfHbO(:,tidx) sbjListPerfHbR(:,tidx) sbjListPerfHbT(:,tidx)]);
    [results,means,~,gnames] = multcompare(stats,"CType","bonferroni");
    ...
    % for high-performance vs low-performance, just use two-sampled two-tailed t-test.
    %[~,pPerf,~,statsPerf] = ttest2(cellGrpPerfHbT{1}(tidx,:),cellGrpPerfHbT{2}(tidx,:));
    [~,pPerf,~,statsPerf] = ttest2(sbjListGoodPerfHbT(:,tidx),sbjListBadPerfHbT(:,tidx));
    ...
    labelHbX = {'Δ[HbO]','Δ[HbR]','Δ[HbT]'};
    %labelPerf = {'High-Performance','Low-Performance','All'};
    labelPerf = {'High-Performance','Low-Performance'};
    ...
    x = [1:1:3];
    x2 = [1:2];
    dHbX = [grpPerfHbO{3}(tidx) grpPerfHbR{3}(tidx) grpPerfHbT{3}(tidx)];
    %dPerf = [grpPerfHbT{1}(tidx) grpPerfHbT{2}(tidx) grpPerfHbT{3}(tidx)];
    dPerf = [grpPerfHbT{1}(tidx) grpPerfHbT{2}(tidx)];
    errHbXH = [abs(squeeze(grpPerfCIHbO{3}(1,tidx))) abs(squeeze(grpPerfCIHbR{3}(1,tidx))) abs(squeeze(grpPerfCIHbT{3}(1,tidx)))];
    errHbXL = [abs(squeeze(grpPerfCIHbO{3}(1,tidx))) abs(squeeze(grpPerfCIHbR{3}(1,tidx))) abs(squeeze(grpPerfCIHbT{3}(1,tidx)))];
%     errPerfH = [abs(squeeze(grpPerfCIHbT{1}(1,tidx))) abs(squeeze(grpPerfCIHbT{2}(1,tidx))) abs(squeeze(grpPerfCIHbT{3}(1,tidx)))];
%     errPerfL = [abs(squeeze(grpPerfCIHbT{1}(1,tidx))) abs(squeeze(grpPerfCIHbT{2}(1,tidx))) abs(squeeze(grpPerfCIHbT{3}(1,tidx)))];
    errPerfH = [abs(squeeze(grpPerfCIHbT{1}(1,tidx))) abs(squeeze(grpPerfCIHbT{2}(1,tidx)))];
    errPerfL = [abs(squeeze(grpPerfCIHbT{1}(1,tidx))) abs(squeeze(grpPerfCIHbT{2}(1,tidx)))];
    bar(x,dHbX);
    er = errorbar(x,dHbX,errHbXL,errHbXH);
    er.Color = [0 0 0];
    er.LineStyle = 'none';
    if numClasses == 2
        yline(.5,'--');
    ...
    bar(x2,dPerf);
    er = errorbar(x2,dPerf,errPerfL,errPerfH);
    er.Color = [0 0 0];
    er.LineStyle = 'none';
    if numClasses == 2
        yline(.5,'--');
