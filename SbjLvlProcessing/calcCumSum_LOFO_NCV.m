...
% This is nested cross-validation. Add repetition in inner folds.

function [perf_Subset,predLabels_Subset] = ...
    calcCumSum_LOFO_NCV(sbjNum,trialsTr,trialsTst,movieListTrain,movieListTest,numClasses,idxChn,mlActAuto,timePt)
...
trPerf = zeros(nRept,length(timePt));
trPerf_LOFO = zeros(nRept,length(timePt),size(idxChn,2));
...
for iR = 1:nRept
    ...
    trPerf(iR,:) = sum(trLabels_All,2)/size(trLabels_All,2);
    
    for iT = 1:length(timePt)
        trLabels_LOFO = squeeze(predictedLabelsLOFO(iT,:,:))==movieListTrain(:,2)';
        trPerf_LOFO(iR,iT,:) = sum(trLabels_LOFO,2)/size(trLabels_LOFO,2);
    end
    
end

trPerfMean = mean(trPerf,1);
trPerf_LOFO_Mean = squeeze(mean(trPerf_LOFO,1));
...
for iT = 1:length(timePt)
    ...
    idxFeatFilt = idxChn(trPerfMean(iT) - trPerf_LOFO_Mean(iT,:) >= 0);
