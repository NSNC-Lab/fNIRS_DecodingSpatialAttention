% SYNTAX:
% [data_yavg, data_yavgstd, nTrials, data_ynew, data_yresid, data_ysum2, beta_blks, yR_blks, bvar, hmrstats, betaSS_blks, dof_blks, sse_blks] = hmrR_GLM_HbT_V02(data_y, stim, probe, mlActAuto, Aaux, tIncAuto, rcMap, trange, glmSolveMethod, idxBasis, paramsBasis, rhoSD_ssThresh, flagNuisanceRMethod, driftOrder, c_vector, condList)
%
% UI NAME:
% GLM_HRF_Drift_SS
%
...
%               idxBasis=2. [tau sigma T] applied to both HbO, HbR & HbT
%                  or [tau1 sigma1 T1 tau2 sigma2 T2 tau3 sigma3 T3]
%                  where the 1 (2) indicates the parameters for HbO (HbR).
%                  default: 0.1 3.0 10.0 1.8 3.0 10.0 0.1 3.0 10.0
%               idxBasis=3 [tau sigma T] applied to both HbO, HbR & HbT
%                  or [tau1 sigma1 T1 tau2 sigma2 T2 tau 3 sigma3 T3]
%                  where the 1 (2) indicates the parameters for HbO (HbR).
%               idxBasis=4 [p q T] applied to both HbO, HbR & HbT
%                  or [p1 q1 T1 p2 q2 T2 p3 q3 T3]
%                  where the 1 (2) indicates the parameters for HbO (HbR).
...
%           [0 1 -1 0]. Support multiple rows.
%           e.g. c_vector = [0 1 -1 0;1 0 -1 0;0 0 1 -1];
%       OR
%   struct.
...
%       e.g. c_vector.betaList = [4 5]
%            c_vector.condList = [0 1 -1;1 0 -1; 1 -1 0];
%       condListF must be empty to use c_vector. Likewise, c_vector must be
...
%
% USAGE OPTIONS:
% GLM_HRF_Drift_SS_Concentration: [data_yavg, data_yavgstd, nTrials, data_ynew, data_yresid, data_ysum2, beta_blks, yR_blks, bvar, hmrstats, betaSS_blks, dof_blks, sse_blks] = hmrR_GLM_HbT_V02(data_y, stim, probe, mlActAuto, Aaux, tIncAuto, rcMap, trange, glmSolveMethod, idxBasis, paramsBasis, rhoSD_ssThresh, flagNuisanceRMethod, driftOrder, c_vector, condList)
%
% PARAMETERS:
% trange: [-2.0, 20.0]
% glmSolveMethod: 1
...
function [data_yavg, data_yavgstd, nTrials, data_ynew, data_yresid, data_ysum2, ...
    ...
    hmrR_GLM_HbT_V02(data_y, stim, probe, mlActAuto, Aaux, tIncAuto, rcMap, trange, glmSolveMethod, idxBasis, paramsBasis, rhoSD_ssThresh, flagNuisanceRMethod, driftOrder, c_vector, condList)
...
for iBlk=1:length(data_y)
    ...
    origLstCond = lstCond;
    if exist('condList','var')
        if size(condList,1)==1 && sum(~ismember(condList,[0 1])) == 0
            nCond = sum(condList);
...
%             end
    ...
    elseif idxBasis==3
        ...
        % Should have 9 just to be safe but test for case 3 or 6
        if length(paramsBasis)==2
            nConc = 1;
        elseif length(paramsBasis)==4
            nConc = 2;
...
%             end
    ...
    elseif idxBasis==4
        % AFNI Gamma function. Will yield complex number due to negative 
        %irrational number raised to power.
        if length(paramsBasis)==2
            nConc = 1;
        elseif length(paramsBasis)==4
            nConc = 2;
