% STATUS: incompleted & experimental
% Not for publication.
% 
% SYNTAX:
% preprocessFNIRS06_LOFO_Subset_MultiOnly(s,numClasses,rejTrOp,rejChnOp,...
%   fefOnlyOp,deltaOp,rerunOp,plotOp)
% 
% DESCRIPTION:
...
% deltaOp - int: option to use delta features. Codes for this option are 
%   commented out and not tested.
...
function preprocessFNIRS06_LOFO_Subset_MultiOnly(s,numClasses,rejTrOp,rejChnOp,fefOnlyOp,deltaOp,rerunOp,plotOp)
...
fs = 50;
%timeLenFull = [0.06 0.1 0.2 0.5 1 1.5 2 3 4 5].*fs;
...
if rerunOp
    ...
    performanceLDALedoitHbT_All_Seq = zeros(nrept,length(timePt));
    %performanceLDALedoitHbT_All = zeros(nrept,length(timePt));
    performanceLDALedoitHbT_Subset = zeros(nrept,length(timePt));
    %performanceLDALedoitHbT_Final = zeros(nrept,length(timePt));
    if deltaOp
...
%         % forward selection by default
%         
%         fun = @(XT,yT,Xt,yt)train_RLDA_Ledoit_TrTst_AnonFun(XT,yT,Xt,yt,...
%             numChn,numClasses,mlActAuto);
% 
%         [fs,hist] = sequentialfs(fun,X,y,'cv',cvp);
%         performanceLDALedoitHbT_All_Seq(iRep,iT) = 1-hist.Crit(end);
        
        % Leave-one-feature-out selection
        for iFold = 1:kFold
...
%     diffAll = mean(performanceLDALedoitHbT_All_FS)-mean(performanceLDALedoitHbT_Subset,1);
...
if plotOp
    ...
    for i = 1:length(timeLen)
        ...
        temp = performanceLDALedoitHbT_Subset(:,i);
        se = std(temp(:))/sqrt(nrept*kFold);
