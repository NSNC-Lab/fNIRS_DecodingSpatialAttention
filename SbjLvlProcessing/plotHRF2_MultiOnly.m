...
% Completed
% This plot all conditions from one channel.
function plotHRF2_MultiOnly(sbjNum)
...
try
    ...
    performanceArrMultiHbO = zeros(100,100);
    performanceArrMultiHbR = zeros(100,100);
    disp('CA file doesn''t exist');
end

[maxCAHbO,idxMaxCAHbO] = max(performanceArrMultiHbO,[],2);
[maxCAHbR,idxMaxCAHbR] = max(performanceArrMultiHbR,[],2);
...
for i1 = 1:length(srcIdxGrp)
    for j = 1:length(srcIdxGrp{i1})
        ...
        title(sprintf('S%sD%s R: %s CAM: %s at %ss',...
            srcStr,detectorStr,num2str(R(srcIdxGrp{i1}(j),1)),...
            num2str(maxCAHbO(chnNum)),num2str(idxMaxCAHbO(chnNum)/2)));
        ...
        title(sprintf('S%sD%s R: %s CAM: %s at %ss',...
            ...
            num2str(maxCAHbR(chnNum)),num2str(idxMaxCAHbR(chnNum)/2)));
