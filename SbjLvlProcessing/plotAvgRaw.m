% Plot raw intensity using outputs from scripting
...
for i1 = 1:numConds
    ...
    for i2 = 1:size(stim(1,i1).data,1)
        ...
        for i3 = 1:numChns
            if endPt < size(snirf1.data.dataTimeSeries,1)
                chnXtrials830(i3,i2,:) = squeeze(snirf1.data.dataTimeSeries(startPt:endPt,i3));
                chnXtrials670(i3,i2,:) = squeeze(snirf1.data.dataTimeSeries(startPt:endPt,i3+42));
            end
        end
    end
...
for i1 = 1:length(srcIdxGrp)
    for j = 1:length(srcIdxGrp{i1}) % source 1
        ...
        title(sprintf('Raw S%sD%s %s',srcStr, detectorStr, pruned));
